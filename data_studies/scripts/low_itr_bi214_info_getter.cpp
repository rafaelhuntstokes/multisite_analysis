...
/*
Script is called for every Bi214 event where ITR < some cut (see condor_submission script)
to investigate systematic shift caused by low ITR events in the Bi214 sample.

Want to save the time residual information for the events as well as the usual
energy, position, multisite etc. to see if we can identify what these events 
are.
*/
...
void evaluate_discriminants(int run_number, float fv_cut, float z_cut, int event_type){
    ...
    */
    ...
    std::string output_fname = "/data/snoplus3/hunt-stokes/multisite_clean/data_studies/extracted_data/" + event_string + "/reprocessed_7.0.15_discriminants_2.5_3.125_tres/" + std::to_string(run_number) + ".root";
    ...
    // create a TTree to save the hit level info & event level info
    TTree *info = new TTree("hit_level", "hit_level");
    ...
    info->Branch("tRes", &time_residuals);
    ...
    // load up the gtid list of low ITR events for this run
    std::string gtid_file_name = "/data/snoplus3/hunt-stokes/multisite_clean/data_studies/extracted_data/bi214/gtid_lists/low_itr/" + std::to_string(run_number) + ".txt";
    std::ifstream gtid_file(gtid_file_name);
    ...
    for (int ifile = 0; ifile < filelist.size(); ifile++){
        ...
        */
        ...
        for (int ientry = 0; ientry < num_entries; ientry++){
            ...
            // check gtid is matching a LOW ITR tagged event
            bool low_itr = std::find(gtids.begin(), gtids.end(), gtid) != gtids.end();
            // if (low_itr == false){
            //     // skip event!
            //     continue;
            // }
            ...
            for (size_t ipmt = 0; ipmt < calibratedPMTs.GetCount(); ipmt++){
                ...
                tres = timeResCalc.CalcTimeResidual(pmtCal, event_position_recon, recon_time, true);
                
                time_residuals.push_back(tres);
