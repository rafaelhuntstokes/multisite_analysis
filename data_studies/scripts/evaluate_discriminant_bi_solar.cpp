...
void evaluate_discriminants(int run_number, float fv_cut, float z_cut, int event_type, bool mc_flag = false){
    /*
    ...
    */
    ...
    if (mc_flag == false){
        if (event_type == 1){
            event_in_string = "reprocessed_bi214_ratds_7.0.15";
            event_out_string = "reprocessed_bi214_multisite_7.0.15";
            tree_name = "bi214";
            energy_low = 1.25;
            energy_high = 3.0;
        }
        if (event_type == 2){
            event_in_string = "reprocessed_solar_ratds_7.0.15";
            event_out_string = "reprocessed_solar_multisite_7.0.15_highE_PDF";
        ...
        std::string mc_path = "/data/snoplus3/SNOplusData/production/miniProd_RAT-7-0-14_ASCI_RATHS_newRecoordination/ratds";
        if (event_type == 1){
            event_in_string = mc_path + "/simulationBiPo214_" + std::to_string(run_number) + "*.root";
            event_out_string = "bi214_mc_multisite";
            tree_name = "bi214";
            energy_low = 1.25;
            energy_high = 3.0;
        }
        if (event_type == 2){
            event_in_string = mc_path + "/simulationB8_solar_nue_" + std::to_string(run_number) + "*.root";
            event_out_string = "solar_mc_multisite_highE_PDF";
            tree_name = "above_5MeV";
            energy_low = 5.0;
            energy_high = 12.0;
        }
    ...
    std::string working_dir, input_fname, output_fname;
    working_dir  = "/data/snoplus3/hunt-stokes/multisite_clean/data_studies/extracted_data/better_tagging";
    if (mc_flag == false){
        input_fname  = working_dir + "/" + event_in_string + "/" + std::to_string(run_number) + "*.root";
        output_fname = working_dir + "/" + event_out_string + "/" + std::to_string(run_number) + ".root";
    }
    else {
        input_fname  = event_in_string;
        output_fname = working_dir + "/" + event_out_string + "/" + std::to_string(run_number) + ".root";
    }
    ...
    TH1D *pdf_B8_multi = dynamic_cast<TH1D*>(PDF_B8->Get("multi_4.375_5.0"));
    TH1D *pdf_Tl208_multi = dynamic_cast<TH1D*>(PDF_Tl208->Get("multi_4.375_5.0"));
    ...
    double energy, x, y, z, dlogL, itr, nhitsScaled, HC_ratio, posFOM;
    Int_t nhitsCleaned, nhitsRaw, gtid;
    UInt_t posFOM_hits;

    info->Branch("nhitsCleaned", &nhitsCleaned);
    info->Branch("nhitsRaw", &nhitsRaw);
    // info->Branch("nhitsScaled", &nhitsScaled)
    info->Branch("energy", &energy);
    info->Branch("itr", &itr);
    info->Branch("HC_ratio", &HC_ratio);
    info->Branch("posFOM", &posFOM);
    info->Branch("posFOM_hits", &posFOM_hits);
    info->Branch("x", &x);
    info->Branch("y", &y);
    info->Branch("z", &z);
    ...
    for (int ifile = 0; ifile < filelist.size(); ifile++){
        ...
        */
        ...
        for (int ientry = 0; ientry < num_entries; ientry++){
            ...
            // nhitsScaled  = DetectorStateCorrection::ApplyCorrectionNhits( GetNhitsCleaned() )
            ...
            // obtain data quality variables (ITR, HC_Ratio, POSFOM)
...
int main(int argc, char* argv[]){
    ...
    // run the analysis!
    evaluate_discriminants(run_number, fv_cut, z_cut, event_type, mc_flag);
}